...
void Matrix::set(double d, int i, int j) { 
	this->dizi[i][j] = d; 
}
double Matrix::get(int i, int j) { 
	return this->dizi[i][j]; 
}
void Matrix::randomD() {
	for (int i = 0; i < this->row; i++)
	{
...
Matrix^ Matrix::operator *(Matrix^ oMatrix) {
	...
	for (int i = 0; i < this->row; i++)
	{
		for (int j = 0; j < oMatrix->column; j++)
		{
			...
			{
				temp->dizi[i][j] = temp->dizi[i][j] + (oMatrix->dizi[k][j] * this->dizi[i][k]);
			}
		}
	}
	return temp;
...
Matrix^ Matrix::operator *(double oDouble) {
	...
	for (int i = 0; i < this->row; i++)
	{
		for (int j = 0; j < this->column; j++)
		{
			temp->dizi[i][j] =  oDouble* this->dizi[i][j];
		}
	}
	return temp;
	temp->~Matrix();
...
Matrix^ Matrix::operator -(Matrix^ oMatrix) {
	...
	for (int i = 0; i < this->row; i++)
	{
		for (int j = 0; j < this->column; j++)
		{
			temp->dizi[i][j] = this->dizi[i][j]- oMatrix->dizi[i][j];
		}
	}
	return temp;
...
Matrix^ Matrix::operator^(double x)
{
	...
	for (int i = 0; i < this->row; i++)
	{
		for (int j = 0; j < this->column; j++)
		{
			temp->set(pow(this->get(i, j), x), i, j);
		}
	}
	return temp;
}
